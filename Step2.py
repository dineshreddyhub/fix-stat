...
if __name__ == '__main__':
    ...
    for line in open(fix_filename, 'r'):
        ...
        for item in line:
            item = item.split('=')
            if item[0] in list(stat.keys()):
                stat[item[0]].append(item[1])
            else:
                stat[item[0]] = [item[1]]
            if item[0]=='55':
                if item[1] in list(stat_prod.keys()):
    ...
    print('buy:')
    print('for buy \nThe minimum is:{} \n The mean is: {} \n The median is:{} \n The maximum is:{}'.format(np.min(buy),np.mean(buy), np.median(buy),np.max(buy)))
    print('for Sell \nThe minimum is:{} \n The mean is: {} \n The median is:{} \n The maximum is:{}'.format(np.min(sell),np.mean(sell), np.median(sell),np.max(sell)))
    print('avg. gain for this is'.format(np.mean(sell) - np.mean(buy)))
    ...
    #stat 5 - occurences of unique orders
    ...
    #stat 6 - most popular order-types
    ...
    for k,v in list(stat_prod.items()):
